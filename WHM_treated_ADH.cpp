...
int GG=559;// Make this clearer for user
...
int L = 3; //Number of aux. compartments at each gametocyte life stage.
	...
	for(int k = 0; k < TL - 1; k++){
		...
		if(isnan(killF)==1){
			cout<<"Feck2 "<<k-DELTA<<" " <<endl;
		//cout<<GUTAM[k] <<" "<< GUTL[k]<<" "<<CENTRALLx[k] << " "<< CENTRALAMx[k]<< " "<< METABOLITEAMx[k]<<endl;
		}
		...
		//Gametocyte dynamics. RK4?
		/*fG1[0] = dt * gf12(0,G[0][k],0,nuv[0],CENTRALAMx[k],METABOLITEAMx[k],CENTRALLx[k],&thetaPD);
		for(int a = 1; a < L * 2; a++){
			uu1 = (a-1)/L;
			uu2 = a/L;
			fG1[a] = dt * (gf12(G[a-1][k],G[a][k],nuv[uu1],nuv[uu2],CENTRALAMx[k],METABOLITEAMx[k],CENTRALLx[k],&thetaPD));
		}
		for(int a = L * 2; a < L * 4; a++){
			uu1 = (a-1)/L;
			uu2 = a/L;
			fG1[a] = dt * (gf34(G[a-1][k],G[a][k],nuv[uu1],nuv[uu2],CENTRALAMx[k],METABOLITEAMx[k],CENTRALLx[k],&thetaPD));
		}
		for(int a = L * 4; a < L * 5; a++){
			uu1 = (a-1)/L;
			uu2 = a/L;
			fG1[a] = dt * (gf5(G[a-1][k],G[a][k],nuv[uu1],nuv[uu2],CENTRALAMx[k],METABOLITEAMx[k],CENTRALLx[k],&thetaPD));
		}

		fG2[0] = dt * gf12(0,G[0][k] + 0.5 * fG1[0],0,nuv[0],CENTRALAMx[k],METABOLITEAMx[k],CENTRALLx[k],&thetaPD);
		for(int a = 1; a < L * 2; a++){
			uu1 = (a-1)/L;
			uu2 = a/L;
			fG2[a] = dt * (gf12(G[a-1][k] + 0.5 * fG1[a-1],G[a][k] + 0.5 * fG1[a],nuv[uu1],nuv[uu2],CENTRALAMx[k],METABOLITEAMx[k],CENTRALLx[k],&thetaPD));
		}
		for(int a = L * 2; a < L * 4; a++){
			uu1 = (a-1)/L;
			uu2 = a/L;
			fG2[a] = dt * (gf34(G[a-1][k] + 0.5 * fG1[a-1],G[a][k] + 0.5 * fG1[a],nuv[uu1],nuv[uu2],CENTRALAMx[k],METABOLITEAMx[k],CENTRALLx[k],&thetaPD));
		}
		for(int a = L * 4; a < L * 5; a++){
			uu1 = (a-1)/L;
			uu2 = a/L;
			fG2[a] = dt * (gf5(G[a-1][k] + 0.5 * fG1[a-1],G[a][k] + 0.5 * fG1[a],nuv[uu1],nuv[uu2],CENTRALAMx[k],METABOLITEAMx[k],CENTRALLx[k],&thetaPD));
		}

		fG3[0] = dt * gf12(0,G[0][k] + 0.5 * fG2[0],0,nuv[0],CENTRALAMx[k],METABOLITEAMx[k],CENTRALLx[k],&thetaPD);
		for(int a = 1; a < L * 2; a++){
			uu1 = (a-1)/L;
			uu2 = a/L;
			fG3[a] = dt * (gf12(G[a-1][k] + 0.5 * fG2[a-1],G[a][k] + 0.5 * fG2[a],nuv[uu1],nuv[uu2],CENTRALAMx[k],METABOLITEAMx[k],CENTRALLx[k],&thetaPD));
		}
		for(int a = L * 2; a < L * 4; a++){
			uu1 = (a-1)/L;
			uu2 = a/L;
			fG3[a] = dt * (gf34(G[a-1][k] + 0.5 * fG2[a-1],G[a][k] + 0.5 * fG2[a],nuv[uu1],nuv[uu2],CENTRALAMx[k],METABOLITEAMx[k],CENTRALLx[k],&thetaPD));
		}
		for(int a = L * 4; a < L * 5; a++){
			uu1 = (a-1)/L;
			uu2 = a/L;
			fG3[a] = dt * (gf5(G[a-1][k] + 0.5 * fG2[a-1],G[a][k] + 0.5 * fG2[a],nuv[uu1],nuv[uu2],CENTRALAMx[k],METABOLITEAMx[k],CENTRALLx[k],&thetaPD));
		}

		fG4[0] = dt * gf12(0,G[0][k] + fG3[0],0,nuv[0],CENTRALAMx[k],METABOLITEAMx[k],CENTRALLx[k],&thetaPD);
		for(int a = 1; a < L * 2; a++){
			uu1 = (a-1)/L;
			uu2 = a/L;
			fG4[a] = dt * (gf12(G[a-1][k] + fG3[a-1],G[a][k] + fG3[a],nuv[uu1],nuv[uu2],CENTRALAMx[k],METABOLITEAMx[k],CENTRALLx[k],&thetaPD));
		}
		for(int a = L * 2; a < L * 4; a++){
			uu1 = (a-1)/L;
			uu2 = a/L;
			fG4[a] = dt * (gf34(G[a-1][k] + fG3[a-1],G[a][k] + fG3[a],nuv[uu1],nuv[uu2],CENTRALAMx[k],METABOLITEAMx[k],CENTRALLx[k],&thetaPD));
		}
		for(int a = L * 4; a < L * 5; a++){
			uu1 = (a-1)/L;
			uu2 = a/L;
			fG4[a] = dt * (gf5(G[a-1][k] + fG3[a-1],G[a][k] + fG3[a],nuv[uu1],nuv[uu2],CENTRALAMx[k],METABOLITEAMx[k],CENTRALLx[k],&thetaPD));
		}
